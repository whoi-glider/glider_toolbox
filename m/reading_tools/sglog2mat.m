...
%% Parse the file.
try
    version = textscan(fid,'version: %s\n', 1, 'ReturnOnError', false, 'Delimiter', '');
    % Read header tags:
    frewind(fid);
    ver = str2double(version{1});
    if ver >= 66.11
        header_map = { ...
            'version' 'version: %s\n'
            'revision' 'revision: %s\n'
    ...
    % GPS string format changes in later versions - date string is present
    % in all GPS lines
    if ver >= 66.11
        nonum_param_map{2,2} = nonum_param_map{4,2};
        nonum_param_map{3,2} = nonum_param_map{4,2};
    end
    ...
    while ~feof(fid)
        ...
        ii = find(strcmp(param, rename_param_list), 1);
        if ii
            field = rename_field_list{ii};
        else
            field = param;
        end
        ii = find(strcmp(param, nonum_param_list), 1);
        if ii
            value_list = nonum_funct_list{ii}(rest);
        else
            % This is 2.5x faster than STR2DOUBLE provided that numbers are real.
            value_list = sscanf(sprintf(' %s', rest{:}), '%f')';
        end
        ii = find(strcmp(param, mulval_param_list), 1);
        if ii
            member_list = mulval_member_list{ii};
        else
            member_list = cell(0,1);
        end
        ii = find(strcmp(param, meta_param_list), 1);
        if ii
            which_param_list = meta_which_list{ii};
            [~, mulval_map_index] = ...
                intersect(mulval_param_list, which_param_list);
            mulval_member_list(mulval_map_index) = {matlab.lang.makeValidName(value_list)};

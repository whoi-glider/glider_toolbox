...
%  along with this program.  If not, see <http://www.gnu.org/licenses/>.
  ...
  options.ctd_list = ...
    struct('conductivity',        {'sci_water_cond'        'm_water_cond'}, ...
           ...
           'pressure_conversion', {@bar2dbar               @bar2dbar});
  options.oxygen_list = ...
    struct('oxygen_concentration', {'sci_oxy4_oxygen'}, ...
           'oxygen_saturation',    {'sci_oxy4_saturation'}, ...
           'temperature',          {'sci_oxy4_temp'}, ...
           'time',                 {'sci_oxy4_timestamp'},...
           'calphase',             {'sci_oxy4_calphase'});
  options.optics_list = ...
  ...
  options.extra_sensor_list = ...
    struct('nitrate', {'sci_suna_nitrate_um'});
  ...
  for oxygen_choice_idx = 1:numel(oxygen_choice_list)
    ...
    if all(ismember({oxy_con_field oxy_sat_field}, field_list)) ...
      ...
      if ~isempty(calphase_oxygen_field) ...
          && ismember(calphase_oxygen_field, field_list) ...
          && any(data_raw.(calphase_oxygen_field) > 0)
        data_pre.calphase_oxygen = data_raw.(calphase_oxygen_field);
        meta_pre.calphase_oxygen.sources = calphase_oxygen_field;
        fprintf('  calphase oxygen  : %s\n', calphase_oxygen_field);
      end
      break;
    end
  end
